...
cipher_suite = Fernet(
    settings.ENCRYPTION_SECRET_KEY.encode()
    if isinstance(settings.ENCRYPTION_SECRET_KEY, str)
    else settings.ENCRYPTION_SECRET_KEY
)
...
def decrypt_api_keys(encrypted_str: str) -> dict:
    ...
    try:
        ...
        # Gracefully handle decryption failures (e.g. key rotation or corrupt data)
        return {}
