...
export default function RepositoryExplorerView() {
  ...
  const executeRepoScan = async () => {
    ...
    try {
      const res = await fetch(apiUrl("/api/v1/repo/analyze"), {
        method: "POST",
        headers: apiHeaders(),
      ...
      console.error("Repository scanning engine fault:", err);
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          <div>
            <h1 className="text-xl font-semibold tracking-tight">
              Repository Target Architecture Ingest
            </h1>
            <p className="text-xs text-zinc-500">
              Analyze full remote directories to evaluate system health, security posture, and
              design structure.
            </p>
          </div>
          ...
          <div className="flex space-x-3 items-center bg-zinc-900/40 border border-zinc-800/80 p-2.5 rounded-xl shadow-inner">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") executeRepoScan();
              }}
            ...
            <button
              ...
              disabled={isScanning}
              className="flex items-center space-x-1.5 px-4 py-1.5 bg-zinc-100 hover:bg-zinc-200 text-zinc-950 text-xs font-medium rounded-lg transition-all shadow-md disabled:opacity-50"
            >
              {isScanning ? (
          ...
          {error && (
            <div className="p-4 rounded-xl border border-red-900/40 bg-red-950/20 text-xs text-red-400 font-mono">
              {error}
            </div>
          )}
          ...
          {repoMetrics && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      {repoMetrics.frameworks && repoMetrics.frameworks.length > 0 ? (
                        repoMetrics.frameworks.map((fw: string, idx: number) => (
                          <span
                            key={idx}
                            className="px-3 py-1 bg-zinc-900 border border-zinc-800 rounded-full text-xs font-medium text-indigo-300"
                          >
                            {fw}
                          </span>
                        ))
                        ...
                        <span className="text-xs text-zinc-500 font-mono">
                          Standard repository structural layout
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-5 rounded-2xl border border-zinc-800/60 bg-zinc-900/20 backdrop-blur-md space-y-3">
                    ...
                    <div className="text-sm font-mono text-zinc-200">
                      {repoMetrics.architecture_detected}
                    </div>
                    <p className="text-xs text-zinc-400 leading-relaxed">
                      Evaluated across{" "}
                      <span className="text-zinc-200 font-semibold">
                        {repoMetrics.total_files}
                      </span>{" "}
                      active source files. The codebase exhibits clear modular boundaries suited
                      for automated static analysis and continuous delivery pipelines.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-2xl border border-zinc-800/60 bg-zinc-900/20 backdrop-blur-md space-y-3">
                  <div className="text-xs font-semibold text-zinc-400 uppercase tracking-wide">
                    File Extension Spectrum
                  </div>
                  <div className="space-y-1.5 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                    {repoMetrics.file_distribution &&
                      Object.entries(repoMetrics.file_distribution).map(
                        ([key, value]: [string, any]) => (
                          <div
                            key={key}
                            className="flex items-center justify-between text-xs font-mono border-b border-zinc-900/60 pb-1.5"
                          >
...
function MetricCard({
  ...
  return (
    <div
      className={`p-5 rounded-2xl border ${border} bg-zinc-900/20 backdrop-blur-md space-y-3 shadow-lg`}
    >
      <div className="text-xs font-semibold text-zinc-400 uppercase tracking-wide flex items-center space-x-2">
        {icon}
        <span>{title}</span>
